...
2. 還原 = 演練腳本指向真的叢集與資料庫名:
   ...
   它會 `DROP DATABASE IF EXISTS` 目標庫——對正在用的 `exchange` 這是刻意的,所以第 1 步要先停角色。腳本以 superuser `pg_restore --no-owner --role=ex_migrate` 還原、驗 migration 版本、跑 `scripts/sql/restore-checks.sql`(試算平衡、餘額快取 = postings、每市場 seq = max(orders.seq)、成交 idx 無洞、每 entry ≥ 2 postings)並印 RTO。
