...
- **還原檢查寫進去的不變量要先問「誰會消耗 seq」。** `restore-checks.sql` 原本斷言 `market_sequences.last_seq = max(orders.seq)`,但 cancel 消耗一個 seq 卻不寫 order 列;loadgen 每一輪都以 place 結尾、e2e 的 burst 檢查也是,所以本機與 burst 之後都過,直到 CI 的 drill 跑在 Playwright smoke 的 cancel 之後才紅(run #82,而 #81 是被 `tee` 蓋掉的假綠)。現在以市場最新的 outbox 事件 seq 為準、orders 只當下界。
