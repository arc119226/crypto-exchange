...
同一輪裡,sweeper 從同一個充值地址規劃了兩筆歸集:USDC(需要先補 gas)和 ETH。ETH 那筆確認後把地址上的 ether 掃走了——**連剛補過去當 gas 的那筆一起**。USDC 那筆在簽名前重新檢查,發現地址已經付不起自己的手續費,於是放棄:
...
**沒有錢遺失,也沒有送出任何註定失敗的交易。** 失敗那筆的 gas 欄是空的,因為它從來沒被簽名——`fail(..., money.Zero, nil)`。真正的代價是那一筆補 gas 交易自己的手續費(約 0.000023 ETH),而補過去的 ether 本身被 ETH 那筆一起掃回熱錢包了。

自我修復是**有界的**:11:07:24 失敗,11:11:25 重新規劃,一個歸集週期。不是無限重試。

這個競態在 anvil 上物理上不可能出現——確認是瞬間的,`stillAffordable` 的 gas 檢查從來不會不通過。**修法待定**:規劃階段要嘛把代幣 sweep 的 gas 預算從原生 sweep 的金額裡扣掉,要嘛在同一個地址還有代幣 sweep 等著時延後原生那筆。兩種都比現在好,但這一節先只記錄現象——修法要在獨立驗證過之後才寫進來。
