...
4d-1 把設定對準了 Sepolia,4d-2 是**一個人照著 `docs/guides/sepolia.md` 從頭走完一次**——領測試幣、部署 MockUSDC、開使用者、充值、歸集、兩條提現路徑、對帳歸零。走的人不是寫這份文件的人,而這正是重點:文件對不對,只有沒有背景知識的讀者能證明。
...
七筆的 tx hash 記在 `docs/guides/sepolia.md` §5(「上一趟實跑的完整紀錄」),每一筆在
`sepolia.etherscan.io` 上都查得到 —— 這一節每個數字都可以被獨立驗證,包括下面 §22.6 那個
靠區塊順序做出的更正。
...
### 22.7.1 收尾時逐條驗 DoD,又找到兩個

八項修完之後照計畫的規矩(「DoD 不過不進下一階段」)把 Phase 4 的 DoD 逐條對一遍,四條裡有兩條沒過。**兩條都不是「做壞了」,是「從來沒做」** —— 而且四個 phase 以來沒有人發現,因為沒有人逐字讀過那一段。

**一、「無私鑰進 log」只做了一半。** 執行期的洩漏掃描存在(`scripts/e2e.sh`,拿 `gen-dev-secrets` 產的已知字串比對所有容器 log)。但 DoD 另外點名的 `slog.LogValuer` 一個都沒有,而 raw tx bytes 有**兩個**藏身處:`signer.Result.RawTx`,以及 base64 過的 `signerbus.wireResponse.RawTx`。第二個更該修 —— 拆分部署才會用到它,而會去拆 signer role 的部署,正是最在意金鑰處理的那一種。

掃描擋不到它們:掃描比對的是它**事先知道的**字串,而簽名後的交易每次都不一樣。**兩半是互補的,不是重複的。**

**二、`*ecdsa.PrivateKey` 那一條寫不出來。** Go 不允許替別的 package 的型別定義方法,`crypto/ecdsa` 不是我們的。DoD 這一條**在 Go 裡不可實作**,只能改成結構性保證:那把金鑰在 `KeystoreSigner.sign` 裡是個區域變數,直接交給簽名呼叫,不進任何 struct。這寫進了 `hdwallet.Derive` 的註解——因為它是唯一會把金鑰交出去的地方。

**三、七筆 tx hash 一個都沒進 repo。** §22.1 記了區塊、gas、價格、成本,唯獨沒有 hash;它們只活在 gitignore 的 `sepoliaresults.md` 裡。這一條的代價是具體的:**§22.6 的更正靠的就是區塊順序**,而在補上之前,repo 裡沒有任何東西能讓下一個人驗證那個更正。一個沒有附證據的主張,正是這一整節在講的問題,而它出現在這一節自己身上。

補的時候用腳本從原始檔抽出來,不手打——七個 66 字元的雜湊,手抄就是等著翻一個字元。補完再用 `gas × price` 重算七筆成本,對得上 §22.1 的 0.000237。
...
| loadgen + 壓測(§3.3、§12) | `exchangectl loadgen`:註冊 + faucet N 帳戶、每帳戶一個 closed-loop goroutine 掛限價單 / 穿價 / 取消,`--ws-clients` / `--private-clients` / `--idle-connections` 量推播延遲與 1,000 連線;報表 nearest-rank 百分位;`docs/loadtest.md` | 行情側在 Phase 6 全部達標(1,030 連線、depth p99 19 ms、500 訂閱者 46,000 則/s 下 44 ms、私有 p99 24 ms、0 gap)——**這句話在 Phase 7 之後只在低負載下成立**,群組提交讓飽和時的 depth / 私有推播變成 638 / 648 ms,見 §26 與 `plan-v1.0.md` §3.3;引擎單市場飽和 ≈ 165 命令/s,`synchronous_commit=off` 只值 10%,瓶頸是每命令約 17 次 DB 往返;`POST p99 < 50 ms` 與 1,000 orders/s 未達,補法(`pgx.Batch`、group commit)記錄給 Phase 7 |
...
| group commit(§1.2 的設計) | `trading/batch.go`:runner 一次撈最多 `ENGINE_BATCH_SIZE`(50)個命令,一筆交易,`BEGIN` + 全部預讀 + `LockMarketSequence`(必須 == 記憶體 seq)+ 每個命令 `SAVEPOINT cmd_i` → hold Check → `book.Apply` → 寫入與**下一個**命令的鎖同一趟;最後 outbox 列 + `SetMarketSequence(from, to)` + `COMMIT`;回覆在 COMMIT **之後**;基礎設施錯誤 → ROLLBACK、dirty、**同一份程式碼逐一重跑**(`trading_batch_fallbacks_total{reason}`);撈取遇到查詢或同帳戶同 `client_order_id` 就停 | 往返:純掛單 17 → **3**、一筆成交 45–55 → **4**、取消 9 → 3(`TestEngineRoundTripBudget` 釘住);單市場 165 → **266 orders/s / 355 命令/s**,`POST p99` 1,244 → 548 ms(`docs/loadtest.md` §8)。**代價寫在這裡才誠實**:事件要等該組 COMMIT 才進 outbox,所以飽和時 depth delta 與私有推播的 p99 從約 20 ms 變成 **638 / 648 ms**,把 §3.3 原本大幅達標的兩列推到未達;旋鈕是 `ENGINE_BATCH_SIZE`,佇列空時退化成單命令。等價測試在 N=1 與 N=50 各跑一份隨機腳本比投影(簿快照、每 seq 的成交、每 entry 的 postings、每帳戶 `account_seq` 連續、trial balance);批內「先成交後取消」用 `touched` 走記憶體、不消耗 seq、不回滾 |
...
| 備份(§12、§16) | Postgres stock image + `archive_command` 只 cp 到本機 volume(`archive_timeout=900`);sidecar image(postgres + `mc`):每 30 s 出貨 WAL、每 24 h `pg_dump -Fc`(角色 `ex_backup`,`pg_read_all_data`)、`mc rm --older-than`、每個結果一列 `admin.backups`;admin role 讀最新成功列 → `backup_last_success_timestamp_seconds{kind}`(`BackupStale` / `WalArchiveStale` / `BackupNeverTaken`)+ `system/status.last_backup` + 後台首頁;`restore-drill.sh` 還原最新 dump 到拋棄式 DB、驗 migration 版本 + `restore-checks.sql`(試算平衡、餘額快取 = postings、市場 seq = 最新的 order / outbox 事件 seq、成交 idx 無洞)、印 RTO;CI `e2e` job 每個 PR `make backup-drill` | 本機檔案模式:25.7 MB / 231k postings / 59k orders,**RTO 8 秒**;`archive_command` 永遠不碰 S3,所以儲存端故障不會卡 Postgres,磁碟滿才會(`DiskAlmostFull`)。WAL 有歸檔但**沒有 base backup**,PITR 只寫文件——這是 beta checklist 的已知缺口。原計畫兩個 sidecar 靠共用 volume 的 `.shipped` 握手,改成一個 image 一個腳本:少一個協定 |
...
- **還原檢查寫進去的不變量要先問「誰會消耗 seq」。** `restore-checks.sql` 原本斷言 `market_sequences.last_seq = max(orders.seq)`,但 cancel 消耗一個 seq 卻不寫 order 列;loadgen 每一輪都以 place 結尾、e2e 的 burst 檢查也是,所以本機與 burst 之後都過,直到 CI 的 drill 跑在 Playwright smoke 的 cancel 之後才紅(run #82,而 #81 是被 `tee` 蓋掉的假綠)。現在以市場最新的 outbox 事件 seq 為準、orders 只當下界。`scripts/e2e.sh` 的 burst 檢查犯的是同一個錯(它自己的 loadgen 每三張單撤一張),在 PR #28 的 run #85 才被抽到,改成同一個不變量。
- **`grep -q` 接在 pipeline 後面、又開著 `pipefail`,是一個會說謊的檢查。** reader 一比中就退出,還在逐行寫的 writer(`docker compose logs`、大字串的 `printf`)收到 SIGPIPE 退 141,pipefail 讓整條 pipeline 變成失敗:e2e 的「engine serves the command bus」在 run #79 這樣紅了(log 裡明明有那一行),同型的密鑰外洩檢查(`printf '%s' "$all_logs" | grep -qF`)理論上也會把「有外洩」判成「沒有」(printf 還在寫時 grep 就退出),本機以 CI 的 log 大小沒有重現出來,但一起改成安全的寫法。修法是先讀進變數再用 here-string 比對;engine 那條本機用逐行 writer 重現 20/20 失敗、改後 0/20。
- **`… | tee log` 沒有 pipefail 的 CI 步驟永遠不會紅。** run #81 的 e2e job 是綠的,`backup-drill-log` 裡卻是 `backup.sh: line 71: $6: unbound variable` 與 `make: *** Error 1`:GitHub 沒指定 `shell:` 時是 `bash -e {0}`,tee 的 0 蓋掉了 make 的 1,還原演練其實從來沒在 CI 跑成功過。修 sidecar(`record()` 的第六個參數可省略)之外,那一步加上 `shell: bash`(顯式指定才有 `-o pipefail`);本機這次拿得到 `mc` 與 `minio`,S3 模式的 `backup.sh once` → `restore-drill.sh` 真跑一遍(14 秒)才 push。
...
- **拿延遲換吞吐的取捨,要記在得到好處的那一段旁邊。** 群組提交把吞吐拉了 2.1 倍,這一段原本只寫了這件事;而它同時把飽和時的推播與 depth delta 從約 20 ms 推到 638 / 648 ms,也就是把 §3.3 原本大幅達標的兩列變成未達。`docs/loadtest.md` §8.4 當下記了,但那份記錄要求「寫進 beta checklist」的動作一直沒有人做,結果**唯一寫著這件事的地方是壓測報告的一個小節**,而讀 §3.3、讀這一段、讀 checklist 的人都看不到。收斂的時候補上三處。
