...
- **`reversed` 反向含 fee 沒有做,而且做不了。** §12 Phase 8 的充值任務後半要求「已入帳的充值被深度 reorg 之後,反向分錄要連 fee 一起反向」。查證的結果是:`reversed` 這個狀態在 0009 的 CHECK 裡、在後台的篩選清單裡、有事件 schema 也有 golden envelope,**但沒有任何程式寫得出這個狀態**,掃描器的 `rewind` 刻意跳過已入帳的充值(理由寫在原註解裡:錢可能已經被花掉)。所以偵測那一半蓋好了,反應那一半不存在。這是 **Phase 4 的缺口**,不是 Phase 8 的;整條路徑另開一個 PR,§12 那一項維持 `- [~]`。
