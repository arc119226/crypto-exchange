...
| [`docs/guides/self-hosted-runner.md`](guides/self-hosted-runner.md) | 把 CI 搬到自己的機器:為什麼私有倉庫的帳單是額度的 7.8 倍、Windows 11 + WSL2 的完整步驟(Docker 引擎兩條路線、兩個閒置逾時、自動登入與重開機恢復;Linux VPS 亦可)、`vars.CI_RUNNER` 一鍵切換與回退、共用 daemon 的安全清理 |
