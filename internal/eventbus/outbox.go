...
func (Outbox) Append(ctx context.Context, tx pgx.Tx, e Envelope) (int64, error) {
	p, err := outboxParams(ctx, e)
	if err != nil {
		return 0, err
	}
	id, err := sqlcgen.New(tx).InsertOutbox(ctx, p)
	if err != nil {
...
// QueueAppend queues Append's insert into b (one statement, no round trip
// of its own). ctx is the producer's: its trace context goes into the
// row's headers, so each event of a batch of commands still hangs under
// the request that produced it.
func (Outbox) QueueAppend(ctx context.Context, b *pg.Batch, e Envelope) error {
	p, err := outboxParams(ctx, e)
	if err != nil {
		return err
	}
	b.QueueExec(sqlcgen.InsertOutbox,
		p.EventID, p.EventType, p.SchemaVersion, p.TenantID, p.MarketID, p.AccountID, p.Seq, p.AccountSeq,
		p.Subject, p.Headers, p.Payload, p.OccurredAt, p.CorrelationID, p.CausationID)
	return nil
}

// outboxParams validates the envelope and builds the row.
func outboxParams(ctx context.Context, e Envelope) (sqlcgen.InsertOutboxParams, error) {
	if err := e.Validate(); err != nil {
		return sqlcgen.InsertOutboxParams{}, err
	}
	...
	if err != nil {
		return sqlcgen.InsertOutboxParams{}, fmt.Errorf("eventbus: headers: %w", err)
	}
	var seq *int64
	if e.Seq != nil {
