...
func TestMigrateSeedAndRegistry(t *testing.T) {
	...
	assert.Equal(t, 20, strings.Count(out.String(), "applied"), out.String()) // 0001 schemas, 0002 registry, 0003 ledger, 0004 audit, 0005 trading, 0006 eventbus, 0007 auth, 0008 chain addresses, 0009 chain deposits, 0010 chain withdrawals, 0011 chain signing, 0012 chain sweeps, 0013 reconciliation, 0014 chain anchor, 0015 one sweep per address, 0016 webhook, 0017 webhook replay, 0018 admin sessions, 0019 ledger breaks, 0020 webhook rotate secret
